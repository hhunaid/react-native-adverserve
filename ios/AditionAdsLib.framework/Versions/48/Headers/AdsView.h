...
@interface AdsPage : NSObject

@end

@interface AdsTimings : NSObject

- (long)getWebViewInitDuration;

- (long)getRequestDuration;

- (long)getTemplateSubstitutionDuration;

- (long)getWebViewLoadDuration;

- (long)getCompleteAdLoadingDuration;
...
/**
 *  Determine whether the web view should start rendering even if not added to view hierarchy.
 */
- (void)enableEarlyRendering:(BOOL)enable;
...
/**
 *  Calls the completion handler with `window.document.querySelectorAll(«selector»).length`,
 *  as the number of DOM elements addressed by the given CSS selector.
 */
- (void)numberOfElementsWithSelector:(NSString *)inCssSelector  completionHandler:(void (^)(unsigned int count, NSError *error))inCompletionHandler;

/**
 *  Calls the completion handler with `window.document.querySelectorAll(«selector»)[«index»].innerHTML.trim()`,
 *  as the HTML contents of the indicated DOM element in the NodeList addressed by
 *  the given CSS selector.
 *
 *  • Calls the completion handler with nil if the selector addresses no elements
 *    or the given index is otherwise out of range.
 */
- (void)htmlOfElementWithSelector:(NSString *)inCssSelector atIndex:(unsigned int)inIndex  completionHandler:(void (^)(NSString *htmlElement, NSError *error))inCompletionHandler;

/**
 *  Calls the completion handler with `window.document.querySelectorAll(«selector»)[«index»].innerText.trim()`,
 ...
 *    or the given index is otherwise out of range.
 */
- (void)textOfElementWithSelector:(NSString *)inCssSelector atIndex:(unsigned int)inIndex  completionHandler:(void (^)(NSString *elementText, NSError *error))inCompletionHandler;
...
- (void)setPage:(AdsPage *)inAdsPage;

- (AdsTimings *)getTimings;

- (void)click;

- (void)clickWithURL:(NSURL *)inURL;
...
/**
 *  Callback to specify if an ad can open the browser without requiring a user interaction first.
 */
- (BOOL)adCanOpenBrowserWithoutUserInteraction:(AdsView *)inAdsView;
